...
def findorf(outputFolder):
    ...
    fileFasta = padrao = os.path.join(outputFolder, "*_nonDNA.fasta")
    ...
    for nonDNA_file in nonDNA_files:
        ...
        orfipy_gc1 = ["orfipy", "--outdir", saida, entrada, "--bed", f"{nome}_ORFgc1.bed"]

        subprocess.run(orfipy_gc1, check=True)

        # Executa orfipy - genetic code 05
        orfipy_gc5 = ["orfipy", "--table", "5", "--outdir", saida, entrada, "--bed",  f"{nome}_ORFgc5.bed"]

        subprocess.run(orfipy_gc5, check=True)

        # Executa orfipy - genetic code 11
        orfipy_gc11 = ["orfipy", "--table", "11", "--outdir", saida, entrada, "--bed",  f"{nome}_ORFgc11.bed"]
