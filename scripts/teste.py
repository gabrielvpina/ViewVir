...
for arquivo_path in arquivos:
    ...
    for arquivo in lista_arquivos:
        ...
        # Executa orfipy
        orfipy_gc1 = [
            "orfipy", "--outdir", saida, entrada, "--bed", f"{nome}_ORFsGC1.bed"
        ]
        subprocess.run(orfipy_gc1, check=True)

        # Executa orfipy
        orfipy_gc5 = [
            "orfipy", "--table", "5", "--outdir", saida, entrada, "--bed",  f"{nome}_ORFsGC5.bed"
        ]
   ...
   # Iterar sobre os arquivos BED
    ...
    for i in range(len(fasta_bed)):
            ...
            if contig not in listGC1:
                if tamanho >= 150:
                    ...
                    dicioGC1[contig][inicio:final+1] = (inicio, final, frame)
    ...
    # Processamento dos arquivos FASTA
     output_subfolder_fasta = os.path.join(output_subfolder, "ORFs.fasta")  # Arquivo de saída de FASTA processado
    ...
    for record in SeqIO.parse(fasta_file, "fasta"):
        ...
        if contig in dicioGC1:
            ...
            for interval in sorted(dicioGC1[contig]):
                inicio, final, frame = interval.begin, interval.end - 1, interval.data[2]
                features.append(GraphicFeature(start=inicio, end=final, strand=frame, label=contig, color="#ffd700"))
 ...
 ###########################################################################################################################
    ...
    # Iterar sobre os arquivos BED
    bed_folder = os.path.dirname(bedgc5_file)
    for bedgc5_file in os.listdir(bed_folder):
        if bed_file.endswith("ORFsGC5.bed"):
            bed_file_path = os.path.join(bed_folder, bed_file)
            fasta_bed = pd.read_table(bed_file_path, header=None, sep="\t")
    ...
    for i in range(len(fasta_bed)):
            ...
            if contig not in listGC5:
                if tamanho >= 150:
                    ...
                    dicioGC5[contig][inicio:final+1] = (inicio, final, frame)
                    listGC5.append(contig)
            else:
                if tamanho > 150:
                    current_intervals = list(dicioGC5[contig])
                    if not current_intervals:
                        dicioGC5[contig][inicio:final+1] = (inicio, final, frame)
                    else:
                        max_interval = max(current_intervals, key=lambda x: x.end - x.begin)
                        if (final - inicio) > (max_interval.end - max_interval.begin):
                            dicioGC5[contig].remove(max_interval)
                            dicioGC5[contig][inicio:final+1] = (inicio, final, frame)  
 ...
 ###################################################################################################################################
    ...
    # Lista para armazenar todos os GraphicRecord
    all_record_graphics = []

    # Plotagem dos gráficos individuais e acumulação dos GraphicRecord
    for record in SeqIO.parse(fastagc5, "fasta"):
        contig = str(record.id)
        if contig in dicioGC5:
            length = len(record.seq)
        ...
        for interval in sorted(dicioGC5[contig]):
            inicio, final, frame = interval.data
            features.append(GraphicFeature(start=inicio, end=final, strand=frame, color="#ffd700"))

        record_graphic = GraphicRecord(sequence_length=length, features=features)
        all_record_graphics.append(record_graphic)
        
        # Plot do gráfico padrão
        ax, _ = record_graphic.plot(figure_width=5)
        plot_output_path = os.path.join(out_subfoldergc5, f'sequence_{contig}.pdf')
        ax.figure.savefig(plot_output_path, bbox_inches='tight')

# Gerar o plot em múltiplas páginas para todos os GraphicRecord
multipage_plot_path = os.path.join(out_subfoldergc5, 'multipage_sequences.pdf')
GraphicRecord.plot_on_multiple_pages(all_record_graphics, multipage_plot_path, nucl_per_line=7000, lines_per_page=10)


        
        # Gerar o plot em múltiplas páginas
        #multipage_plot_path = os.path.join(out_subfoldergc5, f'multipage_sequence_{contig}.pdf')
        #record_graphic.plot_on_multiple_pages(
            #multipage_plot_path,
            #nucl_per_line=length,  # Definir o número de nucleotídeos por linha como o comprimento da sequência
            #lines_per_page=len(features),  # Definir o número de linhas como o número de sequências (features)
        #)
