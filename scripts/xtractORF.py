                ...
                for record in SeqIO.parse(file_path, "fasta"):
                    ...
                    if contig in dicionario:
                        ...
                        for interval in sorted(dicionario[contig]):
                            ...
                            protein_seq = Seq.translate(nucleotide_seq, to_stop=True)
                            fasta_arch.write(f">{contig}.{numerador}\n{protein_seq}\n")
                            numerador += 1

def generate_graphics(fasta_folder, dicionario, output_folder):
    for fasta_file in os.listdir(fasta_folder):
        if fasta_file.endswith(".fasta"):
            file_path = os.path.join(fasta_folder, fasta_file)
            for record in SeqIO.parse(file_path, "fasta"):
                ...
                if contig in dicionario:
                    ...
                    individual_pdf_path = os.path.join(output_folder, f'sequence_{contig}.pdf')
                    ax.figure.savefig(individual_pdf_path, bbox_inches='tight')
...
def main(bed_folder, fasta_folder, output_folder):
    ...
    generate_graphics(fasta_folder, dicionario, output_folder)
